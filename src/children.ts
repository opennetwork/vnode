...
async function* childrenUnion(childrenGroups: AsyncIterableLike<AsyncIterable<AsyncIterable<VNode>>>): AsyncIterable<AsyncIterable<VNode>> {
  yield asyncIterable(
    [
      {
        reference: Fragment,
        children: asyncExtendedIterable(childrenGroups)
          .map(children => asyncIterable([
            {
              reference: Fragment,
              children
            }
